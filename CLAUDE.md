...
1. Запустить `bash sync-memory.sh pull` — загрузить свежую память из облака (тихо, без вывода)
2. Залогиниться в бэкенд (`POST https://artika.onrender.com/api/v1/auth/login`)
3. Получить проекты + задачи + кампании
4. Показать короткую сводку пользователю
5. Спросить "Что делаем?"

Пользователь может написать **"статус"** — Claude покажет текущее состояние всех проектов.

## Команды менеджера проекта (быстрые)

| Команда | Действие |
|---------|---------|
| `статус` | Полная сводка: проекты, задачи, токены, деплой |
| `утро` | Утренний брифинг (запустить morning-briefing.sh) |
| `задачи` | Список задач по статусам из бэкенда |
| `метрика [проект]` | Данные Яндекс Метрики по проекту |
| `кампании` | Статус рекламных кампаний VK + Директ |
| `деплой` | Push + проверить Render и Vercel |
| `vk токен` | Обновить VK Access Token (vk-token-refresh.sh) |
| `отчёт` | Сгенерировать еженедельный отчёт (weekly-report.sh) |
| `приложение` | Открыть https://frontend-rho-five-49.vercel.app |

## Агенты и автоматизация

Все агенты находятся в папке `agents/`:
- `morning-briefing.sh` — утренний статус (задачи, VK токен, системы)
- `vk-token-refresh.sh` — автообновление VK токена каждые 23ч
- `status-check.sh` — быстрая проверка всех систем
- `weekly-report.sh` — еженедельный отчёт по пятницам
- `AGENTS.md` — полная документация всех агентов

Hooks настроены в `~/.claude/settings.json`:
- **Stop** → автосохранение памяти в GitHub
- **PostToolUse (Bash)** → логирование деплоев

## Субагенты (параллельная работа)

При сложных запросах Claude запускает несколько агентов параллельно:
- Метрика по нескольким проектам → несколько API запросов одновременно
- Анализ + кампании + задачи → параллельный сбор данных
- Это ускоряет ответ в 3-4 раза
